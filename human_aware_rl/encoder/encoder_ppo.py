...
PPO_DATA_DIR = 'data/encoder_sp_runs/'#@
...
from human_aware_rl.baselines_utils import get_vectorized_gym_env, create_model, update_model, save_baselines_model, \
    load_baselines_model, get_agent_from_saved_model,create_model_encoder,update_model_encoder
...
def save_ppo_model(model, save_folder,train_mode=None):
    delete_dir_if_exists(save_folder, verbose=True)

    simple_save(
        tf.get_default_session(),
        save_folder,
        inputs={"obs": model.act_model.X, "pre_obs": model.act_model.pre_obs,
                "action_reward": model.act_model.action_reward},
        outputs={
...
def ppo_run(params):
    ...
    for seed in params["SEEDS"]:
        ...
        gym_env.self_play_randomization = 0 if params["SELF_PLAY_HORIZON"] is None else 1  # @ 0
        ...
        configure_other_agent(params, gym_env, mlp, mdp)  # sp self_play_randomization=1??

        ob_space = gym_env.observation_space
        ac_space = gym_env.action_space
        print(ob_space, ac_space)
        print(gym_env.trajectory_sp)  # True
        print(gym_env.self_play_randomization)  # 1
        ...
        with tf.device('/device:GPU:{}'.format(params["GPU_ID"])):
            ...
            model = create_model_encoder(gym_env, "ppo_agent", **params)
        # Train model
        params["CURR_SEED"] = seed
        train_info = update_model_encoder(gym_env, model, **params)
        # Save model
        ...
        print( curr_seed_dir + "training_info")#data/latent_runs/latent_ppo_sp_simple/seed2229/training_info
        train_infos.append(train_info)
